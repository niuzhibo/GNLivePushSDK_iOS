...
/**
 Image file type.
 */
typedef NS_ENUM(NSUInteger, GNYYImageType) {
    GNYYImageTypeUnknown = 0, ///< unknown
    GNYYImageTypeJPEG,        ///< jpeg, jpg
    GNYYImageTypeJPEG2000,    ///< jp2
    GNYYImageTypeTIFF,        ///< tiff, tif
    GNYYImageTypeBMP,         ///< bmp
    GNYYImageTypeICO,         ///< ico
    GNYYImageTypeICNS,        ///< icns
    GNYYImageTypeGIF,         ///< gif
    GNYYImageTypePNG,         ///< png
    GNYYImageTypeWebP,        ///< webp
    GNYYImageTypeOther,       ///< other image format
};
 ...
 */
typedef NS_ENUM(NSUInteger, GNYYImageBlendOperation) {
    
    /**
     All color components of the frame, including alpha, overwrite the current
     contents of the frame's canvas region.
     */
    GNYYImageBlendNone = 0,
 ...
 */
@interface GNYYImageFrame : NSObject <NSCopying>
...
@property (nonatomic) GNYYImageDisposeMethod dispose;     ///< Frame dispose method.
@property (nonatomic) GNYYImageBlendOperation blend;      ///< Frame blend operation.
...
/**
 ...
 Example:
    ...
    GNYYImageDecoder *decoder = [GNYYImageDecoder decoderWithData:data scale:2.0];
    UIImage image = [decoder frameAtIndex:0 decodeForDisplay:YES].image;
 
    // Decode image during download:
    NSMutableData *data = [NSMutableData new];
    GNYYImageDecoder *decoder = [[GNYYImageDecoder alloc] initWithScale:2.0];
 ...
 */
@interface GNYYImageDecoder : NSObject

@property (nullable, nonatomic, readonly) NSData *data;    ///< Image data.
@property (nonatomic, readonly) GNYYImageType type;          ///< Image data type.
 ...
 */
- (nullable GNYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay;
...
/**
 ...
 @discussion It supports encoding single frame image with the type defined in GNYYImageType.
 It also supports encoding multi-frame image with GIF, APNG and WebP.
 
 Example:
    
    GNYYImageEncoder *jpegEncoder = [[GNYYImageEncoder alloc] initWithType:GNYYImageTypeJPEG];
    jpegEncoder.quality = 0.9;
    [jpegEncoder addImage:image duration:0];
    NSData jpegData = [jpegEncoder encode];
 
    GNYYImageEncoder *gifEncoder = [[GNYYImageEncoder alloc] initWithType:GNYYImageTypeGIF];
 ...
 */
@interface GNYYImageEncoder : NSObject

@property (nonatomic, readonly) GNYYImageType type; ///< Image type.
 ...
 */
- (nullable instancetype)initWithType:(GNYYImageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImage:(UIImage *)image type:(GNYYImageType)type quality:(CGFloat)quality;
...
/**
 ...
 @return The image data, or nil if an error occurs.
 */
+ (nullable NSData *)encodeImageWithDecoder:(GNYYImageDecoder *)decoder type:(GNYYImageType)type quality:(CGFloat)quality;
...
@interface UIImage (GNYYImageCoder)
...
CG_EXTERN GNYYImageType GNYYImageDetectType(CFDataRef data);

/// Convert GNYYImageType to UTI (such as kUTTypeJPEG).
CG_EXTERN CFStringRef _Nullable GNYYImageTypeToUTType(GNYYImageType type);

/// Convert UTI (such as kUTTypeJPEG) to GNYYImageType.
CG_EXTERN GNYYImageType GNYYImageTypeFromUTType(CFStringRef uti);

/// Get image type's file extension (such as @"jpg").
CG_EXTERN NSString *_Nullable GNYYImageTypeGetExtension(GNYYImageType type);
 ...
 */
CG_EXTERN CFDataRef _Nullable YYCGImageCreateEncodedData(CGImageRef imageRef, GNYYImageType type, CGFloat quality);


/**
 Whether WebP is available in GNYYImage.
 */
CG_EXTERN BOOL GNYYImageWebPAvailable();
 ...
 */
CG_EXTERN NSUInteger GNYYImageGetWebPFrameCount(CFDataRef webpData);
...
typedef NS_ENUM(NSUInteger, GNYYImagePreset) {
    GNYYImagePresetDefault = 0,  ///< default preset.
    GNYYImagePresetPicture,      ///< digital picture, like portrait, inner shot
    GNYYImagePresetPhoto,        ///< outdoor photograph, with natural lighting
    GNYYImagePresetDrawing,      ///< hand or line drawing, with high-contrast details
    GNYYImagePresetIcon,         ///< small-sized colorful images
    GNYYImagePresetText          ///< text-like
};
...
/**
 ...
 @param preset        Preset for different image type, default is GNYYImagePresetDefault.
